...
const normalizeZoneMeta = (zone: Zone, issueDate: string | undefined, pageNumber: number, zoneIndex: number): Zone => {
  ...
  return {
    ...
    linkedArticleSlug: zone.linkedArticleSlug?.trim() ? zone.linkedArticleSlug : autoMeta.linkedArticleSlug,
  };
};
...
export default function ZoneEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleZonePick = useCallback(
    (zoneId: string | null) => {
      ...
      if (pendingConnection.sourcePageIndex === selectedPageIndex && pendingConnection.sourceZoneId === zoneId) {
        setSelectedZoneId(zoneId);
        return;
      }
      ...
      const sourcePageNumber = epaper?.pages?.[pendingConnection.sourcePageIndex]?.pageNumber ?? pendingConnection.sourcePageIndex + 1;
      const targetPageNumber = epaper?.pages?.[selectedPageIndex]?.pageNumber ?? selectedPageIndex + 1;
  ...
  const disconnectSelectedZone = useCallback(() => {
    ...
    if (pendingConnection?.sourcePageIndex === selectedPageIndex && pendingConnection.sourceZoneId === selectedZoneId) {
      setPendingConnection(null);
    }
  ...
  const handleSave = async () => {
    ...
    const updatedPages = epaper.pages.map((page, pageIdx) => {
      ...
      return {
        pageImage: typeof page.pageImage === "object" && page.pageImage ? page.pageImage.id || page.pageImage : page.pageImage,
        ...
        articleClips: pageZones.map((zoneData, zoneIdx) => {
          ...
          return {
            ...
            clipImages: Array.isArray(existingClips[zoneIdx]?.clipImages) ? existingClips[zoneIdx].clipImages : [],
          };
        }),
      };
    });
  ...
  const currentImageUrl = currentPageData?.pageImage && typeof currentPageData.pageImage === "object" ? currentPageData.pageImage.url || null : null;
  const selectedPageNumber = epaper?.pages?.[selectedPageIndex]?.pageNumber ?? selectedPageIndex + 1;
  const headerPageData = headerZone ? epaper?.pages?.find((page) => page.pageNumber === headerZone.pageNumber) : null;
  const headerImageUrl = headerPageData?.pageImage && typeof headerPageData.pageImage === "object" ? headerPageData.pageImage.url || null : null;
  const headerCropUrl = headerZone && headerImageUrl ? `/api/epaper/crop?src=${encodeURIComponent(headerImageUrl)}&x=${headerZone.x}&y=${headerZone.y}&w=${headerZone.w}&h=${headerZone.h}` : null;
  const footerPageData = footerZone ? epaper?.pages?.find((page) => page.pageNumber === footerZone.pageNumber) : null;
  const footerImageUrl = footerPageData?.pageImage && typeof footerPageData.pageImage === "object" ? footerPageData.pageImage.url || null : null;
  const footerCropUrl = footerZone && footerImageUrl ? `/api/epaper/crop?src=${encodeURIComponent(footerImageUrl)}&x=${footerZone.x}&y=${footerZone.y}&w=${footerZone.w}&h=${footerZone.h}` : null;
  ...
  const selectedArticleZones = selectedArticleGroup ? zones.filter((zone) => zone.articleGroup === selectedArticleGroup) : selectedZone ? [selectedZone] : [];
